  ...
  try {
    ...
    if (!r.ok) {
      ...
      await fetch(`${base}/rest/v1/logs`, {
        ...
        body: JSON.stringify({ customer_id, event: "api-sites-error", detail: { status: r.status, message: text } }),
      });
    ...
    await fetch(`${base}/rest/v1/logs`, {
      method: "POST",
      headers: { ...headers, "Content-Type": "application/json", Prefer: "return=minimal" },
      body: JSON.stringify({ customer_id, event: "api-sites-ok", detail: { count: Array.isArray(data) ? data.length : 0 } }),
    });
